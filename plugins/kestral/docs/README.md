...
Connect Cursor, Claude Code, Claude Cowork, or Codex to [Kestral](https://app.kestral.ai) — an AI-powered project
management tool for teams. Kestral stores your projects, tasks, documents, and customer feedback, and gives an AI agent
context about all of it.
...
**Recommended:** paste this into Cursor, Claude Code, or Codex. The agent fills `--app`, loads the plugin for that host,
then runs `kestral-setup`.
...
**Desktop:** After the install paste finishes, fully quit the Claude process — not only its window — so the plugin
appears. On macOS use **Claude → Quit** or `Cmd+Q`; on Windows use the app menu or system tray instead of only the `X`.
Reopen Claude, go to **Customize → Plugins → Yours → Kestral**. In **Connectors**, click **Connect**,
then fully quit and reopen Claude again. Start a **new** chat and run `/kestral:kestral-setup`. Open chats keep the
plugin and tool snapshot they started with, so neither the install chat nor the Connect chat will gain Kestral
tools.
...
7. Go to **Customize → Plugins → Yours → Kestral**. In **Connectors**, click **Connect**, then fully
   quit and reopen Cowork again.
...
| Local file upload fails                                                | Kestral uploads through an `app.kestral.ai` URL. Confirm the file exists in the environment where your agent runs and that the app can reach `app.kestral.ai`. A cloud or remote agent cannot read a path that exists only on your laptop; attach the file, use the upload picker the agent opens, or upload it from the Kestral project page. On **Claude Cowork**, go to Settings → Capabilities → enable "Allow network egress" and add `app.kestral.ai`. On **Codex**, go to Settings → Configuration → enable **Allow network access**. |
...
| Claude Code Desktop / Cowork: plugin or tools missing after install    | Fully quit the app process, not only its window. On macOS use **Claude → Quit** or `Cmd+Q`; on Windows use the app menu or system tray instead of only the `X`. Reopen, go to **Customize → Plugins → Yours → Kestral**. In **Connectors**, click **Connect**, then fully quit and reopen again. Start a **new** chat or task and run `/kestral:kestral-setup`; open conversations keep their old plugin and tool snapshot.                                                                                                                                |
...
| Setup script fails                                                     | Read the actionable message in Terminal, then inspect `~/.kestral/setup.log` for the host command output. The file is replaced at the start of every run, so copy it before retrying if you need to preserve the failure.                                                                                                                                                                                                                                                                                                                    |
