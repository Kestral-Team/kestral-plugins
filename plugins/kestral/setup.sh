...
# What `marketplace add` receives: the GitHub repo, or a local marketplace directory when overridden.
MARKETPLACE_SOURCE="$MARKETPLACE_REPO"
...
ENABLE_CURSOR=1
FULL_REINSTALL=0
# Install from a marketplace directory on this machine instead of GitHub, and/or point the
# plugin at a different Kestral endpoint. Both stay out of --help: they exist for testing an
# unpublished marketplace, not for normal installs.
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --no-hooks)
        ...
        ENABLE_HOOKS=0
        ;;
      --local-marketplace-path-override)
        shift
        LOCAL_MARKETPLACE_PATH="${1:-}"
        [ -n "$LOCAL_MARKETPLACE_PATH" ] || abort "--local-marketplace-path-override requires a path"
        ;;
      --mcp-url-override)
        shift
        MCP_URL_OVERRIDE="${1:-}"
        [ -n "$MCP_URL_OVERRIDE" ] || abort "--mcp-url-override requires a URL"
        ;;
      --hooks-only)
        if [ "$ENABLE_HOOKS" -eq 0 ]; then
          abort "Cannot combine --hooks-only and --no-hooks"
...
# Host CLIs log "already enabled" as a failed enable. That is a success for us.
_log_host_cli_output() {
  local _label="$1"
  local _output="$2"
  if printf '%s' "$_output" | grep -qi 'already enabled'; then
    verbose "$_label: already enabled (ok)"
    return 0
  fi
  verbose "$_label: $_output"
}
...
read_tty() {
  ...
  # Redirect stderr first: bash reports a failed >/dev/tty open before a trailing
  # 2>/dev/null is applied, which leaks "Device not configured" when piped.
  if ! printf '%s' "$_prompt" 2>/dev/null >/dev/tty; then
    return 1
  fi
  # shellcheck disable=SC2229
  if ! IFS= read -r "${_var?}" 2>/dev/null < /dev/tty; then
    return 1
  fi
  return 0
}
...
manifest_path = File.join(clone_dir, 'plugins', 'kestral', '.claude-plugin', 'plugin.json')
plugin_root = File.join(clone_dir, 'plugins', 'kestral')
unless File.file?(manifest_path)
  manifest_path = File.join(clone_dir, '.claude-plugin', 'plugin.json')
  plugin_root = clone_dir
end
...
_claude_marketplace_root() {
  local _candidate
  # A directory marketplace is read in place — nothing is cloned under ~/.claude.
  if _is_local_marketplace; then
    printf '%s' "$LOCAL_MARKETPLACE_PATH"
    return 0
  fi
  for _candidate in \
...
_restore_plugin_mcp_json_in_clone() {
  ...
  if [ -n "$_clone_root" ] && [ -d "${_clone_root}/.git" ]; then
    ...
    git -C "$_clone_root" checkout -- plugins/kestral/.mcp.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- .cursor-plugin/mcp.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- plugins/kestral/.cursor-plugin/mcp.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- .claude-plugin/plugin.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- plugins/kestral/.claude-plugin/plugin.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- .codex-plugin/plugin.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- plugins/kestral/.codex-plugin/plugin.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- .cursor-plugin/plugin.json 2>/dev/null || true
    git -C "$_clone_root" checkout -- plugins/kestral/.cursor-plugin/plugin.json 2>/dev/null || true
  fi
}
...
# --- Source overrides (testing an unpublished marketplace) ---

_is_local_marketplace() {
  [ -n "$LOCAL_MARKETPLACE_PATH" ]
}

# Point every install path at the local marketplace directory and/or a different Kestral
# endpoint. Hosts read the directory in place (Claude records it as a "directory" source,
# Codex as a marketplace root), so the directory must stay put after setup finishes.
resolve_source_overrides() {
  if [ -n "$MCP_URL_OVERRIDE" ]; then
    REMOTE_MCP_URL="$MCP_URL_OVERRIDE"
  fi
  ...
  local _resolved
  if ! _resolved="$(cd "$LOCAL_MARKETPLACE_PATH" 2>/dev/null && pwd)"; then
    abort_with_hint "Marketplace directory not found: ${LOCAL_MARKETPLACE_PATH}" \
      "Stage it first, then pass that directory."
  fi
  LOCAL_MARKETPLACE_PATH="$_resolved"

  if [ ! -f "${LOCAL_MARKETPLACE_PATH}/.claude-plugin/marketplace.json" ]; then
    abort_with_hint "No marketplace manifest at ${LOCAL_MARKETPLACE_PATH}/.claude-plugin/marketplace.json" \
      "Pass a staged marketplace directory, not the plugin source folder."
  fi

  MARKETPLACE_SOURCE="$LOCAL_MARKETPLACE_PATH"
  MARKETPLACE_GIT_URL="$LOCAL_MARKETPLACE_PATH"

  section "Source override"
  warn "Installing from ${LOCAL_MARKETPLACE_PATH} instead of ${MARKETPLACE_REPO}"
  log "Kestral endpoint: ${REMOTE_MCP_URL}"
  log "Re-run without --local-marketplace-path-override to go back to the published plugin."
  if [ ! -d "${LOCAL_MARKETPLACE_PATH}/.git" ]; then
    warn "Not a git repository — Claude Cowork installs need one (stage the directory again)."
  fi
}
...
# Plugin files live at plugins/kestral/ in the rendered marketplace (Claude catalog
# source). Older published clones kept them at the marketplace root.
_kestral_plugin_root() {
  local _mkt="${1:-}"
  if [ -z "$_mkt" ] || [ ! -d "$_mkt" ]; then
    return 1
  fi
  if [ -f "${_mkt}/plugins/kestral/.claude-plugin/plugin.json" ]; then
    printf '%s/plugins/kestral' "$_mkt"
    return 0
  fi
  if [ -f "${_mkt}/.claude-plugin/plugin.json" ] \
    || [ -f "${_mkt}/.codex-plugin/plugin.json" ] \
    || [ -f "${_mkt}/.cursor-plugin/plugin.json" ]; then
    printf '%s' "$_mkt"
    return 0
  fi
  return 1
...
_marketplace_has_kestral_plugin() {
  _kestral_plugin_root "${1:-}" >/dev/null 2>&1
}
...
ensure_marketplace() {
  ...
  local _output _rc
  set +e
  _output="$(_run_claude_cmd plugin marketplace add "$MARKETPLACE_SOURCE")"
  _rc=$?
  set -e
...
install_or_update_plugin() {
  ...
  local _output _rc
  # `plugin update` compares versions only, so an edited marketplace at the same version stays
  # stale. Uninstall first: the next install rewrites the versioned cache from the source.
  if _is_local_marketplace; then
    set +e
    _output="$(_run_claude_cmd plugin uninstall "$PLUGIN_ID")"
    _rc=$?
    set -e
    verbose "plugin uninstall output: $_output (rc=$_rc)"
  fi
  set +e
  ...
  # Reinstalling keeps the previous enabled/disabled state, which can leave a fresh install off.
  if _is_local_marketplace && [ "$_rc" -eq 0 ]; then
    set +e
    _output="$(_run_claude_cmd plugin enable "$PLUGIN_ID")"
    _rc=$?
    set -e
    _log_host_cli_output "plugin enable output" "$_output"
    if [ "$_rc" -ne 0 ] && ! printf '%s' "$_output" | grep -qi 'already enabled'; then
      warn "Plugin installed but could not be enabled — enable it with: claude plugin enable ${PLUGIN_ID}"
    fi
    ok "Kestral plugin installed from ${LOCAL_MARKETPLACE_PATH}"
    return 0
  fi
...
_finalize_marketplace_clone() {
  local _clone_dir="$1"
  local _plugin_root
  _plugin_root="$(_kestral_plugin_root "$_clone_dir" || printf '%s' "$_clone_dir")"
  _rewrite_plugin_mcp_json_remote "$_clone_dir"
  # The published tree keeps the plugin under plugins/kestral/, and that nested copy is the
  # one hosts load — its MCP JSON needs the same endpoint as the clone root.
  if [ "$_plugin_root" != "$_clone_dir" ]; then
    _rewrite_plugin_mcp_json_remote "$_plugin_root"
  fi
  _write_plugin_server_json "$_plugin_root"
}
...
print_desktop_success() {
  ...
  if [ "$TARGET_RESULT_CLAUDE" = "installed" ]; then
    printf '  → Also run /reload-plugins in the Claude Code CLI if it was running, so it picks up the updated plugin.\n'
  fi
}
...
Codex manual install (GUI fallback):
  ...
  6. Authenticate: app — Plugins → Kestral → MCP servers gear → Authenticate; CLI — run `codex mcp login Kestral`.
  7. Start a new thread using /new after login (this session will not see Kestral tools), then run $kestral-setup.
EOF
}
...
ensure_codex_marketplace() {
  log "Registering Kestral marketplace..."
  local _output _rc
  # `marketplace upgrade` only refreshes Git snapshots, so re-register the directory instead.
  if _is_local_marketplace; then
    set +e
    _output="$(_run_codex_cmd plugin remove "$PLUGIN_ID")"
    verbose "codex plugin remove output: $_output"
    _output="$(_run_codex_cmd plugin marketplace remove "$MARKETPLACE_NAME")"
    verbose "codex marketplace remove output: $_output"
    _output="$(_run_codex_cmd plugin marketplace add "$MARKETPLACE_SOURCE")"
    _rc=$?
    set -e
    verbose "codex local marketplace add output: $_output (rc=$_rc)"
    if [ "$_rc" -eq 0 ]; then
      ok "Kestral marketplace registered from ${LOCAL_MARKETPLACE_PATH}"
      return 0
    fi
    warn "Failed to register the marketplace from ${LOCAL_MARKETPLACE_PATH}."
    return 1
  fi
  set +e
  _output="$(_run_codex_cmd plugin marketplace add "$MARKETPLACE_SOURCE")"
...
install_or_update_codex_plugin() {
  ...
  _rc=$?
  set -e
  _log_host_cli_output "codex plugin add output (rc=$_rc)" "$_output"
  ...
  if printf '%s' "$_output" | grep -qi 'already installed'; then
    ...
    _rc=$?
    set -e
    _log_host_cli_output "codex plugin re-add output (rc=$_rc)" "$_output"
    if [ "$_rc" -eq 0 ]; then
      ok "Kestral plugin updated"
      return 0
    fi
...
print_codex_success() {
  ...
  printf '  %d. Start a new thread using /new (or + New thread in the app).\n' "$_step"
  _step=$((_step + 1))
  # shellcheck disable=SC2016
  printf '  %d. If Codex says Kestral is not logged in, run: codex mcp login Kestral\n' "$_step"
  _step=$((_step + 1))
  printf '  %d. After login, start a new thread using /new — this session will not see Kestral tools.\n' "$_step"
  _step=$((_step + 1))
  # shellcheck disable=SC2016
  printf '  %d. Type: $kestral-setup — connect Kestral and start your first plan.\n' "$_step"
}
...
# Plugin files Cursor should copy: an overridden marketplace directory wins over the folder
# setup.sh itself lives in, because the marketplace directory carries the rendered manifests.
_setup_script_plugin_root() {
  local _src _dir
  if _is_local_marketplace && [ -f "${LOCAL_MARKETPLACE_PATH}/.cursor-plugin/plugin.json" ]; then
    printf '%s' "$LOCAL_MARKETPLACE_PATH"
    return 0
  fi
  _src="${BASH_SOURCE[0]:-}"
...
_print_cursor_manual_fallback() {
  ...
  printf '  Then run kestral-setup. Quit and reopen Cursor only if the skill is missing.\n'
}
...
print_cursor_success() {
  ...
  printf '  1. Open Settings → Tools & MCPs. If Kestral shows Needs authentication, click Connect.\n'
  printf '  2. In agent chat, run kestral-setup — connect Kestral and start your first plan.\n'
}
...
_plugin_root_for_target() {
  ...
  case "$_target" in
    claude-code)
      ...
      _kestral_plugin_root "$_mkt" 2>/dev/null || printf '%s' "$_mkt"
      ;;
    claude-desktop)
      _mkt="$(_desktop_marketplace_clone_dir 2>/dev/null || true)"
      _kestral_plugin_root "$_mkt" 2>/dev/null || printf '%s' "$_mkt"
      ;;
    codex)
...
_claude_plugin_install_roots() {
  ...
  if [ -n "$_mkt" ] && [ -d "$_mkt" ]; then
    printf '%s\n' "$_mkt"
    if [ -d "${_mkt}/plugins/kestral" ]; then
      printf '%s\n' "${_mkt}/plugins/kestral"
    fi
  fi
...
_print_hooks_next_steps() {
  ...
  while IFS= read -r _target; do
    ...
    case "$_target" in
      claude-code)
        printf '  Claude Code: /reload-plugins so hooks register.\n'
        ;;
      claude-desktop)
...
_print_mcp_auth_next_steps() {
  ...
  if [ "$TARGET_RESULT_CODEX" = "installed" ]; then
    ...
    printf '  Codex CLI: run `codex mcp login Kestral`, then start a new thread using /new (this session will not see Kestral tools after login).\n'
  fi
  if [ "$TARGET_RESULT_CURSOR" = "installed" ]; then
...
_print_summary() {
  ...
  if _is_local_marketplace; then
    warn "Installed from ${LOCAL_MARKETPLACE_PATH} (${REMOTE_MCP_URL}) — not the published plugin."
  elif [ -n "$MCP_URL_OVERRIDE" ]; then
    warn "Kestral endpoint set to ${REMOTE_MCP_URL} instead of the default."
  fi
  _print_mcp_auth_next_steps
  if [ "$HOOKS_FAILED" -eq 1 ]; then
...
when 'rpm_manifest'
  if data['plugins'].is_a?(Array)
    data['plugins'].reject! { |p| p.is_a?(Hash) && p['name'].to_s.downcase.include?('kestral') }
  end
when 'claude_json'
...
# Newer Cowork builds load plugins from a per-account cache of the copy Cowork itself
# uploaded (`rpm/`), not from the marketplace clone under cowork_plugins/. Leaving that
# cache in place means a reinstall changes nothing that Cowork actually reads.
_clean_cowork_remote_plugin_cache() {
  local _dir _file _cleaned=0

  [ -d "$SESSIONS_BASE" ] || return 0

  while IFS= read -r _dir; do
    if grep -q '"name"[[:space:]]*:[[:space:]]*"kestral"' "${_dir}/.claude-plugin/plugin.json" 2>/dev/null \
      || grep -q '"name"[[:space:]]*:[[:space:]]*"kestral"' "${_dir}/.codex-plugin/plugin.json" 2>/dev/null; then
      rm -rf "$_dir"
      verbose "Removed Cowork plugin cache: $_dir"
      _cleaned=1
    fi
  done < <(find "$SESSIONS_BASE" -maxdepth 4 -type d -path '*/rpm/plugin_*' 2>/dev/null)

  while IFS= read -r _file; do
    grep -q -i kestral "$_file" 2>/dev/null || continue
    if _strip_kestral_from_json "$_file" "rpm_manifest"; then
      verbose "Stripped Kestral from $_file"
      _cleaned=1
    fi
  done < <(find "$SESSIONS_BASE" -maxdepth 4 -path '*/rpm/manifest.json' 2>/dev/null)

  [ "$_cleaned" -eq 1 ] || return 0

  ok "Cleared the Kestral copy Cowork had cached"
  warn "Cowork keeps its own copy of Kestral in the cloud and may download it again when it starts."
  printf '  → To stop that, open Cowork → Customize → Plugins, remove Kestral there, then re-run this script.\n'
}
...
_clean_claude_plugins() {
  ...
  # Claude Code keeps its own registry next to the caches; a stale entry here can point at a
  # marketplace directory that no longer exists.
  if [ -n "$RUBY_BIN" ]; then
    if _strip_kestral_from_json "${HOME}/.claude/plugins/known_marketplaces.json" "known_marketplaces"; then
      ok "Removed Kestral marketplaces from ${HOME}/.claude/plugins/known_marketplaces.json"
    fi
    if _strip_kestral_from_json "${HOME}/.claude/plugins/installed_plugins.json" "installed_plugins"; then
      ok "Removed Kestral plugins from ${HOME}/.claude/plugins/installed_plugins.json"
    fi
  fi
